...
function addData(form){
  ...
  if($(".required").length === 0 ){
    ...
    $.ajax({
          ...
          dataType: "json",
          beforeSend: function () {
              $("#msg_table_name").addClass("error");
              $("#"+form+" input[id="+table+"_save_button]").prop("disabled",true);
              $("#msg_table_name").html("Insertando... <img src='../img/loading_fast.gif'  >");
          },
          success: function (data) {   
              ...
              if(data.mensaje === "Ok"){ 
                  ...
                  $("#msg_table_name").html(data.mensaje+" Rellene los campos requeridos y vuelva a intentarlo si el problema persiste contacte con el Administrador del sistema.");
              }           
          },
          error: function (err) { 
            $("#msg_table_name").addClass("error");
            $("#"+form+" input[id="+table+"_save_button]").prop("disabled",false);
            $("#msg_table_name").html('Error al Registrar: Rellene los campos requeridos y vuelva a intentarlo.<a href="javascript:showTechnicalError()">Mas info</a>\n\
            <div class="technical_info">'+err.responseText+'</div>');         
          }
      });  
     }else{
      $("#msg_table_name").addClass("error");  
      $("#msg_table_name").html("Rellene los campos requeridos...");
    }
}
        ...
        $.ajax({
              ...
              success: function (data) {                
                  if(data.mensaje === "Ok"){ 
                      ...
                      $("#msg_table_name").html(data.mensaje+' intente nuevamente si el problema persiste contacte con el Administrador del sistema.<a href="javascript:showTechnicalError()">Mas info</a><div class="technical_info">'+data.query+'</div>');
                  }           
              },
              error: function (err) { 
                $("#msg_table_name").addClass("error");
                $("#msg_table_name").html('Error al Registrar: Verifique los datos y vuelva a intentarlo.<a href="javascript:showTechnicalError()">Mas info</a>\n\
                <div class="technical_info">'+err.responseText+'</div>');         
              }
        }); 
  }else{
