...
class Y_Template {
    ...
    private $file;
    private $json_var;
    private $dynamicVars = []; // Array para almacenar variables dinámicas
    private $t_contents = []; // Array para almacenar los contenidos de los bloques
    public $temp;
...
//			            $temp=addslashes($temp);
    ...
    if ($ident == "0") {
        $this->t_contents[0] = ""; // Reset the "0" content
        $x = "";
        foreach ($this->t_contents as $temp) {
            $temp = addslashes($temp);
            if (preg_match_all('/\{\$this->(\w+)\}/', $temp, $matches)) {
                foreach ($matches[1] as $property) {
                    if (!property_exists($this, $property)) {
                        $this->$property = ''; // Define the property with an empty value
                    }
                }
            }
            eval("\$x= \"$temp\";");
        ...
        // if a block with the priority language exists
